...
export async function recordRegistration(data: RegistrationInput): Promise<void> {
  await sql()`
    INSERT INTO registrations (
      session_id, first_name, last_name, email, phone, firm_name, bar_license, selected_packages
    ) VALUES (
...
export async function listRegistrations(): Promise<RegistrationRow[]> {
  return sql()<RegistrationRow[]>`
    SELECT
      ...
      r.firm_name, r.bar_license, r.selected_packages, r.created_at,
